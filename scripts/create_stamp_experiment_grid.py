...
for dataset_name in dataset_names:
    ...
    gmlp_type = 'criss_cross' # Options: [None, 'old', 'basic', 'criss_cross']
    ...
    tr_type = None # Options: [None, 'basic', 'criss_cross']
    tr_heads_values = [8] if tr_type is not None else None
    tr_n_layers_values = [8] if tr_type is not None else None
    tr_dff_values = [256] if tr_type is not None else None
    ...
    if encoder_aggregation == 'attention_pooling':
        grid_components.update({
            ...
            'mhap_lambda_for_residual': mhap_lambda_for_residual_values
        })
    ...
    for i, combo in enumerate(hyperparameter_combinations, 1):
        ...
        if encoder_aggregation == 'attention_pooling':
            mhap_params = {
                ...
                'lambda_for_residual': mhap_lambda_for_residual
            }
        else:
            mhap_params = None
